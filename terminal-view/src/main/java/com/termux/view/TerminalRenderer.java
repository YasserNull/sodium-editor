...
public final class TerminalRenderer {
  ...
  public TerminalRenderer(float textSize, Typeface typeface) {
    mTextSize = textSize;
    mTypeface = typeface;
  ...
  public final void render(
    ...
    for (int row = topRow; row < endRow; row++) {
      ...
      final boolean baseRtl = isRtlLine(line, charsUsedInLine);
      for (int column = 0; column < columns; ) {
        ...
        final boolean currentCharIsRtl = isRtlChar(codePoint);
        ...
        if (currentCharIsRtl) {
          // Avoid breaking RTL runs into single glyphs; let shaping handle it.
  ...
  private void drawTextRun(
    ...
    int visualStartColumn =
        baseRtl ? (totalColumns - (startColumn + runWidthColumns)) : startColumn;
    float left = visualStartColumn * mFontWidth;
    ...
    float leftPx = Math.round(left);
    float rightPx = Math.round(right);
    if (backColor != palette[TextStyle.COLOR_INDEX_BACKGROUND]) {
      // Only draw non-default background.
      mTextPaint.setColor(backColor);
      canvas.drawRect(leftPx, y - mFontLineSpacingAndAscent + mFontAscent, rightPx, y, mTextPaint);
    }

    if (cursor != 0) {
      mTextPaint.setColor(cursor);
      float cursorHeight = mFontLineSpacingAndAscent - mFontAscent;
      float cursorRight = rightPx;
      if (cursorStyle == TerminalEmulator.TERMINAL_CURSOR_STYLE_UNDERLINE) cursorHeight /= 4.;
      else if (cursorStyle == TerminalEmulator.TERMINAL_CURSOR_STYLE_BAR)
        cursorRight = rightPx - ((rightPx - leftPx) * 3) / 4.f;
      canvas.drawRect(leftPx, y - cursorHeight, cursorRight, y, mTextPaint);
    }
  ...
  private static boolean isRtlChar(int codePoint) {
  ...
  private static boolean isRtlLine(char[] text, int charsUsed) {
    for (int i = 0; i < charsUsed; ) {
      char c = text[i];
      int codePoint = Character.isHighSurrogate(c) ? Character.toCodePoint(c, text[i + 1]) : c;
      byte dir = Character.getDirectionality(codePoint);
      if (dir == Character.DIRECTIONALITY_LEFT_TO_RIGHT) return false;
      if (dir == Character.DIRECTIONALITY_RIGHT_TO_LEFT
          || dir == Character.DIRECTIONALITY_RIGHT_TO_LEFT_ARABIC
          || dir == Character.DIRECTIONALITY_RIGHT_TO_LEFT_EMBEDDING
          || dir == Character.DIRECTIONALITY_RIGHT_TO_LEFT_OVERRIDE) return true;
      i += Character.isHighSurrogate(c) ? 2 : 1;
    }
    return false;
  }
