...
public class RealFileDeleteLineInstrumentedTest {
  ...
  /**
   * This test intentionally catches the "phantom render" issue: after deleting all lines in-memory
   * (unsaved), ViewRender can still populate direct lines from the on-disk file and draw stale
   * text.
   *
   * <p>Expected: direct-line reads should not be used while there are pending edits. Current
   * behavior (bug): direct-line reads can still pull old text.
   */
  @Test
  ...
  private void assertNoPhantomRenderText(String forbidden) throws Exception {
    ...
    for (int i = 0; i < windowSnapshot.size(); i++) {
      ...
      if (v != null && v.contains(forbidden)) {
        throw new AssertionError(
            "Phantom text found in linesWindow at localIdx=" + i + " value='" + v + "'");
      }
    }
    ...
    for (int i = 0; i < linesCount + 6; i++) {
      ...
      if (text != null && text.contains(forbidden)) {
        throw new AssertionError(
            "Phantom text rendered at globalLine=" + i + " value='" + text + "'");
      }
      if (i >= linesCount && text != null && !text.isEmpty()) {
        throw new AssertionError(
            "Non-existent line rendered non-empty at globalLine=" + i + " value='" + text + "'");
      }
    }
    ...
    for (int idx : outOfRange) {
      ...
      if (text != null && !text.isEmpty()) {
        throw new AssertionError(
            "Out-of-range line rendered non-empty at globalLine=" + idx + " value='" + text + "'");
      }
    }
  }
  ...
  private void assertWindowRenderHasNoPhantomText(String forbidden) throws Exception {
    ...
    for (int local = 0; local < win.size(); local++) {
      ...
      if (text != null && text.contains(forbidden)) {
        throw new AssertionError(
            "Phantom text found in linesWindow local="
                + local
                + " global="
                + global
                + " value='"
      ...
      if (global >= linesCount && text != null && !text.isEmpty()) {
        throw new AssertionError(
            "EOF window line is non-empty local="
                + local
                + " global="
                + global
                + " value='"
    ...
    synchronized (editor.windowRender.modifiedLines) {
      for (java.util.Map.Entry<Integer, String> e : editor.windowRender.modifiedLines.entrySet()) {
        ...
        if (text != null && text.contains(forbidden)) {
          throw new AssertionError(
              "Phantom text found in modifiedLines at global=" + line + " value='" + text + "'");
        }
        if (line >= linesCount && text != null && !text.isEmpty()) {
          throw new AssertionError(
              "Out-of-range modifiedLines non-empty at global=" + line + " value='" + text + "'");
        }
      }
    }
  }
