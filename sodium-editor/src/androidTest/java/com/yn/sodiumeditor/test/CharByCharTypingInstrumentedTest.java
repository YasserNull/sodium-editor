...
public class CharByCharTypingInstrumentedTest {
    ...
    public void typeArabicOnly_charByChar_mustInsertEveryChar() throws Exception {
        ...
        assertEquals(text, rendered);
        assertTrue("Snapshot must contain Arabic text", editor.fileIO.getTextSnapshot().contains("مرحبا"));
    }
    ...
    public void typeEnglishAndArabic_charByChar_multiLine() throws Exception {
        ...
        InstrumentationRegistry.getInstrumentation()
                .runOnMainSync(
                        () -> {
                            editor.cursor.setCursorPosition(0, 0);
                            for (int i = 0; i < l0.length(); i++) inputConnection.commitText(String.valueOf(l0.charAt(i)), 1);
                            inputConnection.commitText("\n", 1);
                            for (int i = 0; i < l1.length(); i++) inputConnection.commitText(String.valueOf(l1.charAt(i)), 1);
                        });
        drainUi();
