...
public class HighlightParser {
    ...
    public HighliteRender.LineParseResult parseLineForSyntax(
        ...
        if (strState == Highlite.STRING_STATE_BACKTICK && !highlite.isBacktickStringsEnabled) strState = 0;
        if (strState == Highlite.STRING_STATE_TRIPLE && !highlite.isTripleQuoteStringsEnabled) strState = 0;
        if (strState != 0 && !highlite.isMultiLineStringsEnabled && strState != Highlite.STRING_STATE_TRIPLE) strState = 0;
        ...
        while (i < len) {
            ...
            if (strState != 0) {
                SodiumEditor.StringEndResult res = findStringEndForState(line, i, strState);
                if (res.found) {
            ...
            if (isTripleQuoteStart(line, i) && !HighlightUtils.isEscaped(line, i)) {
                ...
                if (highlite.isTripleQuoteStringsEnabled) {
                    ...
                    return new HighliteRender.LineParseResult(spans, false, Highlite.STRING_STATE_TRIPLE);
                }
            }
    ...
    public int getStringStateForDelimiter(char c) {
        if (c == '"') return Highlite.STRING_STATE_DOUBLE;
        if (c == '\'') return Highlite.STRING_STATE_SINGLE;
        return Highlite.STRING_STATE_BACKTICK;
    }

    public SodiumEditor.StringEndResult findStringEndForState(String line, int start, int state) {
        if (state == Highlite.STRING_STATE_TRIPLE) {
            int end = HighlightUtils.findTripleQuoteEnd(line, start);
            return new SodiumEditor.StringEndResult(end >= 0, end >= 0 ? end + 3 : start);
        }
        char d = (state == Highlite.STRING_STATE_SINGLE) ? '\'' : (state == Highlite.STRING_STATE_BACKTICK ? '`' : '"');
