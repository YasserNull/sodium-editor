...
public class SelectionClipboard {
  ...
  public void copyOrCutSelection(final boolean cut) {
    ...
    boolean fullyInWindow =
        (fsL >= editor.windowRender.windowStartLine)
            && (feL < editor.windowRender.windowStartLine + editor.windowRender.linesWindow.size());

    if (fullyInWindow) {
      String text =
          selection.textBuilder.buildSelectedTextFromWindow(
              fsL, fsC, feL, feC, selection.copyCutMaxChars);
      setPrimaryClip(text);
    ...
    editor.fileIO.ioHandler.post(
        () -> {
          final String text =
              selection.textBuilder.buildSelectedTextBlocking(
                  fsL, fsC, feL, feC, selection.copyCutMaxChars);
          editor.post(
              () -> {
                setPrimaryClip(text);
  ...
  private void setPrimaryClip(String text) {
    ClipboardManager cm =
        (ClipboardManager) editor.getContext().getSystemService(Context.CLIPBOARD_SERVICE);
