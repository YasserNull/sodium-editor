...
public class LineNumber {
  ...
  public void setCurrentLineGutterHighlightEnabled(boolean e) {
    highlightCurrentLineInGutter = e;
    if (showLineNumbers) editor.invalidate();
  }
  ...
  public void setGutterSeparatorColor(int c) {
    gutterSeparatorPaint.setColor(c);
    if (showLineNumbers) editor.invalidate();
  }
  ...
  public void setGutterSeparatorWidth(float w) {
    gutterSeparatorWidth = Math.max(0f, w);
    editor.requestLayout();
    if (showLineNumbers) editor.invalidate();
  }
  ...
  public void setLineNumberTextSize(float s) {
    lineNumbersPaint.setTextSize(s);
    invalidateLineNumberCache();
    editor.requestLayout();
    if (showLineNumbers) editor.invalidate();
  }
  ...
  public void setLineNumberTypeface(@Nullable Typeface tf) {
    lineNumbersPaint.setTypeface(tf != null ? tf : editor.textRender.baseTypeface);
    invalidateLineNumberCache();
    editor.requestLayout();
    if (showLineNumbers) editor.invalidate();
  }
  ...
  public void updateGutterWidth() {
    ...
    if (Math.abs(lineNumbersGutterWidth - old) > 0.1f) {
      invalidateLineNumberCache();
      editor.requestLayout();
      editor.invalidate();
    }
  ...
  public void drawLineNumbersCachedUnwrapped(Canvas canvas, int fI, int lI, int fL, int lL) {
    if (!cache.shouldUseCache()) {
      render.drawLineNumbersDirectUnwrapped(canvas, fI, lI, fL, lL);
      return;
    }
  ...
  public void drawLineNumbersCachedWrapped(Canvas canvas, int fV, int lV) {
    ...
    if (cache.needsRebuild(gw, h, fV, drawLastV, baseY, true)) {
      ...
      for (int v = fV; v <= drawLastV; v++) {
        ...
        if (p.segment == 0)
          render.drawSingleLineNumber(lineNumberCacheCanvas, p.line + 1, lineNumX, v * pad - baseY);
      }
  ...
  public void drawCurrentLineNumberUnwrapped(Canvas canvas, int fI, int lI) {
    ...
    int orig = lineNumbersPaint.getColor();
    lineNumbersPaint.setColor(currentLineNumberColor);
    render.drawSingleLineNumber(
        canvas,
        editor.cursor.cursorLine + 1,
        render.getLineNumXUnwrapped(),
        vI * editor.textRender.lineHeight - editor.scroll.scrollY);
