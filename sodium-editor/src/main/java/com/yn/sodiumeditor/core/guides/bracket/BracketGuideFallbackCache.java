...
/**
 * Manages the fallback cache to prevent flickering during window changes. Uses SparseArray for
 * efficient sparse storage without range tracking.
 */
public class BracketGuideFallbackCache {
  ...
  /**
   * Merges the current main cache with the existing fallback cache. Directly puts main cache
   * entries into SparseArray — no list creation, no loop over ranges.
   */
  ...
  public void shiftCache(int startLine, int delta) {
    ...
    fallbackTokens.clear();
    for (int i = 0; i < shiftedTokens.size(); i++)
      fallbackTokens.put(shiftedTokens.keyAt(i), shiftedTokens.valueAt(i));
    ...
    fallbackStates.clear();
    for (int i = 0; i < shiftedStates.size(); i++)
      fallbackStates.put(shiftedStates.keyAt(i), shiftedStates.valueAt(i));
  }
}
