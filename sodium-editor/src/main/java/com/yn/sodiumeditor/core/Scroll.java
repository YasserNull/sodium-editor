...
public class Scroll {
    ...
    public void computeScroll() {
        if (scroller.computeScrollOffset()) {
            ...
            editor.maybeKickWindowLoad(editor.wordWrap.getGlobalLineForY(scrollY));
    ...
    public void keepCursorVisibleHorizontally() {
        if (editor.zoom.isScaling || (editor.scaleGestureDetector != null && editor.scaleGestureDetector.isInProgress()) || editor.multiTouchActive) return;
        float oldX = scrollX, oldY = scrollY;
        ...
        int vH = editor.getHeight() - editor.keyboardHeight; if (vH <= 0) vH = editor.getHeight();
        float bP = (editor.keyboardHeight > 0) ? getKeyboardBarrierPadding() : getBottomBarrierPadding();
        float evH = Math.max(0f, vH - bP), vT = scrollY, vB = scrollY + evH;
        float nY = scrollY;
        if (yB > vB) nY = yB - evH; else if (yT < vT) nY = yT;
        if (editor.keyboardHeight > 0) {
            float kbT = editor.getHeight() - editor.keyboardHeight, pAK = getKeyboardBarrierPadding(), cCVY = yB - scrollY;
            if (cCVY >= kbT - pAK) nY = yB - (editor.getHeight() - editor.keyboardHeight - pAK);
        }
        float nX = scrollX;
        if (!editor.wordWrap.isWordWrapEnabled) {
            String ln = editor.textRender.getLineTextForRender(editor.cursor.cursorLine);
            float cX = editor.caret.getCaretXForLine(ln, editor.cursor.cursorLine, Math.min(editor.cursor.cursorChar, editor.getLogicalLineLength(editor.cursor.cursorLine, ln)));
            ...
            float sM = 50f, eSX = getEffectiveScrollX(), cVX = editor.getTextStartX() + cX - eSX, minV = vL + sM, maxV = vR - sM;
            if (cVX < minV) eSX = editor.getTextStartX() + cX - minV; else if (cVX > maxV) eSX = editor.getTextStartX() + cX - maxV;
    ...
    public float viewToTextX(float vX) { return vX + getEffectiveScrollX() - editor.getTextStartX(); }
