...
public class AutoCompletion {
    ...
    public void updateSuggestionInternal() {
        ...
        List<TextRender.HighlightSpan> spans = editor.highlightCache.get(editor.cursor.cursorLine);
        if (spans == null) {
            spans = editor.highlite.calculateSpansForLine(line, editor.cursor.cursorLine);
            editor.highlightCache.put(editor.cursor.cursorLine, spans);
        }
  ...
  public void drawAutoSuggestionWrapped(Canvas canvas, String lineContent, int globalLine, int segStart, int segEnd, int visualIndex, float textBaselineY) {
    ...
    float suggestionStartX_canvas = editor.measureTextWithVisualSpaces(lineContent, segStart, cursorPositionInLine, editor.textRender.paint);
    canvas.drawText(activeSuggestion, suggestionStartX_canvas, textBaselineY, suggestionPaint);
