...
/**
 * Basic view-level operations for SodiumEditor. Handles performance mode, backgrounds, fonts,
 * read-only/disable, navigation, and scaling utilities.
 */
public class View {
  ...
  public void applyTypeface(
    ...
    android.graphics.Typeface safeBase =
        (typeface != null) ? typeface : android.graphics.Typeface.DEFAULT;
    ...
    android.graphics.Typeface finalTypeface =
        android.graphics.Typeface.create(safeBase, typefaceStyle);
    ...
    for (com.yn.sodiumeditor.renderer.HighlightRender.HighlightRule rule :
        editor.highlight.highlightRules) {
    ...
    editor.wordWrap.indicator.wordWrapIndicatorWidth =
        editor.wordWrap.indicator.wordWrapIndicatorPaint.measureText(
            editor.wordWrap.indicator.getWordWrapIndicatorText());
  ...
  public void applyTextSizePx(float sizePx, boolean deferWrapRebuild) {
    ...
    editor.autoCompletion.suggestionPaint.setTextSize(
        sizePx * editor.autoCompletion.suggestionTextSizeScale);
    ...
    editor.wordWrap.indicator.wordWrapIndicatorPaint.setTypeface(
    ...
    if (editor.wordWrap.isWordWrapEnabled)
      editor.wordWrap.invalidateWrapMetrics(true, !deferWrapRebuild);
    editor.wordWrap.requestWrapPrefixRebuild();
    editor.invalidate();
  }
  ...
  public void goToLine(int line, int col) {
    ...
    if (editor.fileIO.sourceFile == null || editor.fileIO.isFileCleared) {
      synchronized (editor.windowRender.linesWindow) {
        knownTotal =
            Math.max(
                1, editor.windowRender.windowStartLine + editor.windowRender.linesWindow.size());
      }
  ...
  public void proceedGoToLineClamped(
    ...
    if (editor.fileIO.isWindowLoading
        ...
        && !(targetLine >= editor.windowRender.windowStartLine
            && targetLine
    ...
    Runnable completionAction =
        () -> {
          ...
          if (editor.cursor.cursorLine >= editor.windowRender.windowStartLine
            ...
            if (editor.cursor.cursorLine > lastLineInDoc)
              editor.cursor.cursorLine = Math.max(0, lastLineInDoc);
            String lineText = editor.windowRender.getLineTextForRender(editor.cursor.cursorLine);
            editor.cursor.cursorChar = Math.max(0, Math.min(targetCol, lineText.length()));
          } else {
  ...
  public int getLinesCount() {
    ...
    if (editor.fileIO.isEof)
      return editor.windowRender.windowStartLine + editor.windowRender.linesWindow.size();
  ...
  public void computeWidthForLine(int globalIndex, String line) {
    ...
    if (oldWidth != null && oldWidth >= editor.windowRender.globalMaxLineWidth && w < oldWidth) {
      editor.windowRender.recalculateMaxLineWidth();
    }
  }
    ...
    for (int i = 0; i < textLen; i++) {
      float adv =
          editor.textRender.getCharAdvanceWidth(
              text.charAt(i), measureWidthBuffer[i], editor.textRender.paint);
