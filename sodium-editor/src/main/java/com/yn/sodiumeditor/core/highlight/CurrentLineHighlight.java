...
/**
 * Manages current line highlighting for the SodiumEditor. Enhanced with ultra-smooth sliding
 * animation.
 */
public class CurrentLineHighlight {
  ...
  public void drawCurrentLineHighlightInGutter(Canvas canvas, float top, float bottom) {
    if (!editor.lineNumber.showLineNumbers
        || !highlightCurrentLineInGutter
        || editor.selection.hasSelection) return;

    animation.checkAndStartAnimation();

    float yOffset =
        animation.getAnimatedVisualIndex() * editor.textRender.lineHeight - editor.scroll.scrollY;
    float aTop = yOffset;
  ...
  public void drawCurrentLineHighlightUnwrapped(
      Canvas canvas,
  ...
  public void drawCurrentLineHighlightWrapped(
    ...
    canvas.drawRect(
        -editor.textRender.paddingLeft,
        top,
        Math.max(editor.wordWrap.getWrapWidth(), editor.getWidth()),
        bottom,
        currentLinePaint);
  }

  public void drawCurrentLineHighlightSegment(
