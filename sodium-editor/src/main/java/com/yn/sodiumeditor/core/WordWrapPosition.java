...
public class WordWrapPosition {
    ...
    public WordWrap.VisualLinePosition getVisualPositionForIndexFallback(int visualIndex) {
        ...
        int line = baseLine;
        int windowEnd = Math.max(baseLine, editor.getWindowEndLine());
        while (line <= windowEnd) {
            int[] starts = wordWrap.getWrapStartsForLine(line, editor.textRender.getLineTextForRender(line));
    ...
    public int getVisualIndexForLineAndChar(int line, int ch) {
        ...
        String text = editor.textRender.getLineTextForRender(safeLine);
        ...
        if (editor.codeFold.isCodeFoldingEnabled) {
            visualIndex = editor.codeFold.getVisibleIndexForGlobalLine(line) + (visualIndex - wordWrap.wrapLinePrefix[safeLine]);
        }
        return visualIndex;
    }
    ...
    public EditOp.CursorTarget getCursorTargetForPosition(float viewX, float viewY, @Nullable Map<Integer, String> directLines) {
        ...
        String line = editor.textRender.getLineTextForRenderWithDirect(pos.line, directLines);
        float x = editor.scroll.viewToTextX(viewX);
        if (!wordWrap.isWordWrapEnabled) {
            int charIdx = editor.getCharIndexForX(line, x, pos.line);
            int clamped = Math.max(0, Math.min(charIdx, editor.getLogicalLineLength(pos.line, line)));
