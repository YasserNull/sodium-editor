...
/**
 * Caches bracket and quote positions for bracket matching. This avoids re-scanning the entire file
 * on every edit.
 */
public class BracketCache {
  ...
  public static class BracketPosition {
    ...
    public static char getMatchingBracket(char c) {
      switch (c) {
        ...
        case BRACKET_PAREN_OPEN:
          return BRACKET_PAREN_CLOSE;
        case BRACKET_PAREN_CLOSE:
  ...
  public static class QuotePosition {
    ...
    public QuotePosition(
        int line, int startColumn, int endColumn, char quoteChar, boolean isMultiline) {
      this.line = line;
  ...
  /**
   * Pre-scan the entire file for brackets and quotes. Should be called in background thread BEFORE
   * showing the file. Reads directly from file, not from window buffer. Blocks until complete.
   */
            ...
            try (RandomAccessFile raf = new RandomAccessFile(editor.fileIO.sourceFile, "r")) {
              ...
              for (int lineNum = 0; lineNum < totalLines && myToken == scanToken; lineNum++) {
                ...
                LineBracketInfo info =
                    parseLineInternal(
                        lineNum, line, inBlockComment, stringState, stringQuoteChar, stringTriple);
  ...
  /**
   * Get bracket info for a line, parsing it if not cached. Reads directly from file if not in
   * window buffer.
   */
  ...
  private int getStringState(char quoteChar, boolean isTriple) {
    if (isTriple) {
      if (quoteChar == QUOTE_BACKTICK)
        return com.yn.sodiumeditor.core.highlight.Highlight.STRING_STATE_TRIPLE;
      return com.yn.sodiumeditor.core.highlight.Highlight.STRING_STATE_TRIPLE;
    }
