...
public class SmartSelection {
  ...
  public boolean applySmartDoubleTapSelection(int line, int charIndex, String lineText) {
    ...
    if (quote != null) {
      selection.wordFinder.addSelectionCandidate(
          candidates, quote.start + 1, quote.end, lineText.length());
      selection.wordFinder.addSelectionCandidate(
          candidates, quote.start, quote.end + 1, lineText.length());
    }

    SelectionTextRange bracket =
        selection.quoteFinder.findEnclosingBracketRange(lineText, charIndex);
    if (bracket != null) {
      selection.wordFinder.addSelectionCandidate(
          candidates, bracket.start + 1, bracket.end, lineText.length());
      selection.wordFinder.addSelectionCandidate(
          candidates, bracket.start, bracket.end + 1, lineText.length());
    }
    ...
    boolean sameAnchor =
        (wStart != -1)
            && (line == selection.lastDoubleTapLine
                && wStart == selection.lastDoubleTapWordStart
                && wEnd == selection.lastDoubleTapWordEnd);
    int currentIdx = selection.findSelectionCandidateIndex(line, candidates);
    int nextIdx =
        (sameAnchor || currentIdx >= 0)
            ? (currentIdx >= 0 ? (currentIdx + 1) % candidates.size() : 0)
            : 0;
