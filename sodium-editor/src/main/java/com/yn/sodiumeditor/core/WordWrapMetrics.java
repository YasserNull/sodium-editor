...
public class WordWrapMetrics {
    ...
    public void buildWrapMetricsInMemory() {
        ...
        for (int i = 0; i < total; i++) {
            int c = wordWrap.calculator.computeWrapCountForLine(editor.textRender.getLineTextForRender(i), widthPx, editor.textRender.paint, true);
            counts[i] = c; running += c; prefix[i + 1] = running;
        }
        applyMetrics(counts, prefix, running, widthPx, (editor.textRender.windowStartLine == 0 ? total - 1 : -1));
    }
    ...
    private void buildWrapMetricsFromFile(int token, int widthPx, Paint wrapPaint) {
        ...
        try (BufferedReader br = editor.fileIO.reopenReaderAtStart()) {
            ...
            while (lineIdx < total) {
                ...
                String mod;
                synchronized (editor.textRender.modifiedLines) { mod = editor.textRender.modifiedLines.get(lineIdx); }
