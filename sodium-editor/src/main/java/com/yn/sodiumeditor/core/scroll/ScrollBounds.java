...
public class ScrollBounds {
  ...
  public float getMaxScrollXForClamp() {
    ...
    float candidateMax =
        Math.max(0f, (rawMaxWidth + extraSpace) - (editor.getWidth() - textStartX));
  ...
  public float getMaxScrollYForClamp() {
    if (editor.wordWrap.isWordWrapEnabled
        && !editor.wordWrap.wrapMetricsReady
        && (editor.zoom.isScaling || editor.zoom.mJustFinishedScale)) return scroll.scrollY;
    ...
    float virtualExtra =
        Math.max(editor.windowRender.prefetchLines * editor.textRender.lineHeight, 2000f);
  ...
  public void clampScrollX() {
    ...
    float max =
        (editor.onTouch.pointerDown && scroll.dragMaxScrollX >= 0f)
            ? scroll.dragMaxScrollX
  ...
  public float getBottomBarrierPadding() {
    float base =
        Math.max(
            com.yn.sodiumeditor.renderer.TextRender.BOTTOM_SCROLL_OFFSET,
            editor.textRender.lineHeight * 2f);
    float min =
        Math.max(
            com.yn.sodiumeditor.renderer.TextRender.MIN_BOTTOM_VISIBLE_SPACE,
            editor.textRender.lineHeight * 2f);
  ...
  public float getKeyboardBarrierPadding() {
    ...
    float min =
        (editor.textRender.lineHeight > 0f)
            ? editor.textRender.lineHeight * 2f
            : com.yn.sodiumeditor.renderer.TextRender.MIN_BOTTOM_VISIBLE_SPACE;
    float max =
        (editor.textRender.lineHeight > 0f)
            ? editor.textRender.lineHeight * 3.5f
            : com.yn.sodiumeditor.renderer.TextRender.BOTTOM_SCROLL_OFFSET;
