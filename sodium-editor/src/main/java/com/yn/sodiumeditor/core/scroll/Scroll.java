...
public class Scroll {
        ...
        public void run() {
          ...
          if (autoScrollX != 0 || autoScrollY != 0) {
            ...
            if (!editor.fileIO.isIndexReady
                && !editor.fileIO.isEof
                && editor.fileIO.isWindowLoading) {
              float effectiveHeight =
                  (editor.view.keyboardHeight > 0)
                      ? editor.getHeight() - editor.view.keyboardHeight
                      : editor.getHeight();
            ...
            clampScrollX();
            clampScrollY();
            editor.onTouch.updateHandlePosition(
                editor.onTouch.lastTouchX, editor.onTouch.lastTouchY);
  ...
  public int getFlingOverScrollX() {
    return (flingBounceEnabled)
        ? (flingBounceOverScrollPx >= 0
            ? flingBounceOverScrollPx
            : Math.max(24, Math.round(editor.getWidth() * flingBounceOverScrollFactor)))
        : 0;
  }
  ...
  public int getFlingOverScrollY() {
    return (flingBounceEnabled)
        ? (flingBounceOverScrollPx >= 0
            ? flingBounceOverScrollPx
            : Math.max(24, Math.round(editor.getHeight() * flingBounceOverScrollFactor)))
        : 0;
  }
  ...
  public void computeScroll() {
    if (scroller.computeScrollOffset()) {
      ...
      float mX = getMaxScrollXForClamp(),
          mY = getMaxScrollYForClamp(),
          v = scroller.getCurrVelocity();
      edge.releaseAll();
      if (stretch.stretchOverscrollEnabled) {
        if (scrollY <= 0 && oldY > 0) stretch.absorbStretchY(v, false);
        else if (scrollY >= mY && oldY < mY) stretch.absorbStretchY(v, true);
        if (scrollX <= 0 && oldX > 0) stretch.absorbStretchX(v, false);
        else if (scrollX >= mX && oldX < mX) stretch.absorbStretchX(v, true);
      }
      ...
      if (flingBounceEnabled) {
        ...
        if (scrollX < 0 || scrollX > mX || scrollY < 0 || scrollY > mY) {
          if (scroller.springBack(Math.round(scrollX), Math.round(scrollY), 0, mX, 0, mY)) {
            scrollerIsScrolling = true;
            editor.postInvalidateOnAnimation();
            return;
          }
  ...
  public void scrollToLineFastForSelectAll(int l, int c) {
    if (editor.wordWrap.isWordWrapEnabled
        && (!editor.wordWrap.wrapMetricsReady || editor.wordWrap.wrapLinePrefix == null))
      scrollY = Math.max(0f, (l - 5) * editor.textRender.lineHeight);
    else
      scrollY =
          Math.max(
              0f,
              (editor.wordWrap.getVisualIndexForLineAndChar(l, c) - 5)
                  * editor.textRender.lineHeight);
    clampScrollY();
  }
  ...
  public void keepCursorVisibleHorizontally() {
    if (editor.zoom.isScaling
        ...
        || editor.onTouch.multiTouchActive) return;
    float oldX = scrollX, oldY = scrollY;
    int vIdx =
        Math.max(0, Math.round(editor.caret.getCaretDocumentY() / editor.textRender.lineHeight));
    float yT = vIdx * editor.textRender.lineHeight, yB = yT + editor.textRender.lineHeight;
    int vH = editor.getHeight() - editor.view.keyboardHeight;
    if (vH <= 0) vH = editor.getHeight();
    float bP =
        (editor.view.keyboardHeight > 0) ? getKeyboardBarrierPadding() : getBottomBarrierPadding();
    float evH = Math.max(0f, vH - bP), vT = scrollY, vB = scrollY + evH;
    float nY = scrollY;
    if (yB > vB) nY = yB - evH;
    else if (yT < vT) nY = yT;
    if (editor.view.keyboardHeight > 0) {
      float kbT = editor.getHeight() - editor.view.keyboardHeight,
          pAK = getKeyboardBarrierPadding(),
          cCVY = yB - scrollY;
    ...
    if (!editor.wordWrap.isWordWrapEnabled) {
      ...
      float vR =
          editor.textRender.isRtl
              ...
              : editor.getWidth();
      float sM = 50f,
          eSX = getEffectiveScrollX(),
          cVX = editor.layout.getTextStartX() + cX - eSX,
          minV = vL + sM,
          maxV = vR - sM;
      if (cVX < minV) eSX = editor.layout.getTextStartX() + cX - minV;
      else if (cVX > maxV) eSX = editor.layout.getTextStartX() + cX - maxV;
      float max = getMaxScrollXForClamp();
      eSX =
          Math.max(
              editor.textRender.isRtl ? -max : 0f,
              Math.min(eSX, editor.textRender.isRtl ? 0f : max));
  ...
  public void setScrollMode(int m) {
    if (m == SCROLL_MODE_SINGLE_AXIS || m == SCROLL_MODE_GRID || m == SCROLL_MODE_FREE)
      this.scrollMode = m;
  }
