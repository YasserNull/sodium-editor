...
public class ScrollHandler {
  ...
  public boolean handleScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
    ...
    if (scroll.scrollMode == Scroll.SCROLL_MODE_SINGLE_AXIS) {
      if (scroll.scrollLockAxis == 0)
        scroll.scrollLockAxis = (Math.abs(dx) >= Math.abs(dy)) ? 1 : 2;
      if (scroll.scrollLockAxis == 1) dy = 0f;
      else dx = 0f;
    ...
    float maxX =
        !editor.wordWrap.isWordWrapEnabled
            ? (scroll.dragMaxScrollX = scroll.getMaxScrollXForClamp())
            : 0f;
    ...
    if (scroll.stretch.stretchOverscrollEnabled || scroll.edge.edgeEffectEnabled) {
      if (!editor.wordWrap.isWordWrapEnabled) {
        if (nextX < 0f && dx < 0f) {
          scroll.stretch.pullStretchX(dx, false);
          scroll.edge.pullLeft(dx, e2.getY());
          nextX = 0f;
        } else if (nextX > maxX && dx > 0f) {
          scroll.stretch.pullStretchX(dx, true);
          scroll.edge.pullRight(dx, e2.getY());
          nextX = maxX;
        } else scroll.edge.releaseHorizontal();
      }
      if (nextY < 0f && dy < 0f) {
        scroll.stretch.pullStretchY(dy, false);
        scroll.edge.pullTop(dy, e2.getX());
        nextY = 0f;
      } else if (nextY > maxY && dy > 0f) {
        scroll.stretch.pullStretchY(dy, true);
        scroll.edge.pullBottom(dy, e2.getX());
        nextY = maxY;
      } else scroll.edge.releaseVertical();
    } else {
      if (!editor.wordWrap.isWordWrapEnabled
          && ((scroll.scrollX <= 0f && dx < 0f) || (scroll.scrollX >= maxX && dx > 0f))) {
        dx = 0f;
        nextX = scroll.scrollX;
      }
      scroll.edge.releaseVertical();
      scroll.edge.releaseHorizontal();
    }

    scroll.scrollY = nextY;
    scroll.scrollX = nextX;
    scroll.clampScrollY();
    scroll.clampScrollX();
  ...
  public boolean handleFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
    ...
    if (scroll.scrollMode == Scroll.SCROLL_MODE_SINGLE_AXIS) {
      int axis = scroll.scrollLockAxis;
    ...
    int oX =
        (scroll.flingBounceEnabled && !editor.wordWrap.isWordWrapEnabled)
            ? scroll.getFlingOverScrollX()
            : 0;
