...
public class WordWrapPosition {
  ...
  public WordWrap.VisualLinePosition getVisualPositionForIndex(int visualIndex) {
    ...
    if (!wordWrap.isWrapMetricsUsableForWindow(widthPx)) {
      ...
      return new WordWrap.VisualLinePosition(visualIndex, 0);
    }
  ...
  public WordWrap.VisualLinePosition getVisualPositionForIndexFallback(int visualIndex) {
    ...
    int baseVisual =
        (wordWrap.wrapLinePrefix != null
                && wordWrap.wrapPrefixValidUpToLine >= baseLine
                && baseLine < wordWrap.wrapLinePrefix.length)
            ? wordWrap.wrapLinePrefix[baseLine]
            : baseLine;
    ...
    while (line <= windowEnd) {
      int[] starts =
          wordWrap.getWrapStartsForLine(line, editor.windowRender.getLineTextForRender(line));
      int segCount = Math.max(1, starts.length);
      if (remaining < segCount)
        return new WordWrap.VisualLinePosition(
  ...
  public EditOp.CursorTarget getCursorTargetForPosition(
    ...
    WordWrap.VisualLinePosition pos =
        wordWrap.isWordWrapEnabled
            ? getVisualPositionForIndex(visualIndex)
            : new WordWrap.VisualLinePosition(visualIndex, 0);
    ...
    if (!wordWrap.isWordWrapEnabled) {
      ...
      int clamped =
          Math.max(0, Math.min(charIdx, editor.view.getLogicalLineLength(pos.line, line)));
      return new EditOp.CursorTarget(
          pos.line,
          editor.binaryRender.shouldUseBinaryRenderingForLine(pos.line)
              ? editor.binaryRender.snapBinaryCursor(line, clamped, pos.line)
              : clamped);
    }
    int[] starts = wordWrap.getWrapStartsForLine(pos.line, line);
    int seg = Math.min(Math.max(0, pos.segment), Math.max(0, starts.length - 1));
    int charIdx =
        wordWrap.getCharIndexForXInRange(
            line,
            pos.line,
            wordWrap.getWrapSegmentStart(starts, seg),
            wordWrap.getWrapSegmentEnd(starts, seg, line.length()),
            x);
