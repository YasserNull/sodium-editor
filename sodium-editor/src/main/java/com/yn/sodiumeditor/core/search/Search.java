...
public class Search {
  ...
  public void drawSearchHighlightsForLine(
    ...
    for (int i = 0; i + 1 < spans.length; i += 2) {
      ...
      float left = editor.textRender.measureText(line, start, globalLine);
      float right = editor.textRender.measureText(line, end, globalLine);
  ...
  public SearchMatch findNextSearchMatchInRange(
    ...
    while (true) {
      ...
      String lineText = editor.windowRender.getLineTextForRenderWithDirect(line, direct);
