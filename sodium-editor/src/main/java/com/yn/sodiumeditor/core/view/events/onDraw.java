...
public class onDraw {
  ...
  static boolean shouldClearBeforeDraw(
      boolean hasEditorBackgroundColor, Object editorBackgroundBitmap) {
  ...
  public void onDraw(Canvas canvas) {
    ...
    if (shouldClearBeforeDraw(
        editor.view.hasEditorBackgroundColor, editor.view.editorBackgroundBitmap)) {
      canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
    }
    editor.view.drawEditorBackground(canvas);
    if (editor.scroll.stretch.stretchOverscrollEnabled
        && (editor.scroll.stretch.stretchX != 0f || editor.scroll.stretch.stretchY != 0f)) {
      float sx =
          1f
              + (editor.scroll.stretch.stretchX
                  * 0.18f
                  * editor.scroll.stretch.stretchOverscrollStrength);
      float sy =
          1f
              + (editor.scroll.stretch.stretchY
                  ...
                  * editor.scroll.stretch.stretchOverscrollStrength);
      float pivotX =
          (editor.scroll.stretch.stretchDirX < 0)
              ? 0f
              : (editor.scroll.stretch.stretchDirX > 0
                  ? editor.getWidth()
                  : editor.getWidth() * 0.5f);
      float pivotY =
          (editor.scroll.stretch.stretchDirY < 0)
              ? 0f
              : (editor.scroll.stretch.stretchDirY > 0
                  ? editor.getHeight()
                  : editor.getHeight() * 0.5f);
      canvas.save();
