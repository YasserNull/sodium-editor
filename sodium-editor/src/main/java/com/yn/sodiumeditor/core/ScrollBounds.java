...
public class ScrollBounds {
    ...
    public float getMaxScrollYForClamp() {
        ...
        int lineCount = editor.wordWrap.isWordWrapEnabled ? editor.wordWrap.getTotalVisualLineCount() : (editor.codeFold.isCodeFoldingEnabled ? editor.codeFold.getVisibleLineCount() : Math.max(1, editor.getLinesCount()));
        ...
        if (editor.fileIO.isEof) {
            float pad = (editor.keyboardHeight > 0) ? getKeyboardBarrierPadding() : getBottomBarrierPadding();
            return Math.max(0f, lineCount * editor.textRender.lineHeight - (effectiveHeight - pad));
        }
        float virtualExtra = Math.max(editor.textRender.prefetchLines * editor.textRender.lineHeight, 2000f);
    ...
    public void clampScrollX() {
        ...
        float max = (editor.pointerDown && scroll.dragMaxScrollX >= 0f) ? scroll.dragMaxScrollX : getMaxScrollXForClamp();
    ...
    public void clampScrollY() {
        if (!editor.wordWrap.isWordWrapEnabled && editor.fileIO.isWindowLoading && scroll.scrollY < editor.textRender.windowStartLine * editor.textRender.lineHeight) {
            if (!(scroll.scrollerIsScrolling || scroll.flingStopAnimator != null)) {
                scroll.scrollY = editor.textRender.windowStartLine * editor.textRender.lineHeight;
    ...
    public float getKeyboardBarrierPadding() {
        ...
        return Math.max(min, Math.min(max, editor.keyboardHeight * 0.4f));
    }
}
