...
public class FileIO {
  ...
  public final java.util.LinkedHashMap<Integer, String> directLineCache =
      new java.util.LinkedHashMap<Integer, String>(250, 0.75f, true) {
        ...
        protected boolean removeEldestEntry(java.util.Map.Entry<Integer, String> eldest) {
          ...
          int lastIdx =
              firstIdx + (int) Math.ceil(editor.getHeight() / editor.textRender.lineHeight) + 5;
  ...
  public void checkAndLoadWindow() {
    windowLoader.checkAndLoadWindow();
  }
  ...
  public void ensureLineInWindow(int gL, boolean block) {
    ...
    windowLoader.loadWindowAround(
        Math.max(0, line - editor.windowRender.prefetchLines), null, false);
  }
  ...
  public void cancelAndCloseReader() {
    // Kept for release() compatibility. File-backed readers are now short-lived RandomAccessFile
    // instances.
  }
  ...
  public String readLinePrefixUtf8AtByte(RandomAccessFile raf, long offset, int maxBytes)
      throws Exception {
  ...
  public String readLineSliceAtByte(
    ...
    raf.readFully(buf);
    return editor.binaryRender.isBinarySafeRenderingEnabled()
        ? editor.binaryRender.bytesToControlVisible(buf, buf.length, fileCharset)
        : new String(buf, fileCharset);
  }

  public StreamedCharSlice readLineSliceByChars(
      RandomAccessFile raf, long lineStart, int sC, int eC, boolean needTotal) throws Exception {
  ...
  private void resetStateForNewFile() {
    ...
    if (!editor.binaryRender.binaryFileFeaturePolicyActive)
      editor.binaryRender.clearBinaryDocument();
  ...
  private boolean shouldWarmBracketIndexForOpen() {
    ...
    return editor.bracketGuides.isBracketGuidesEnabled
        || editor.symbolsMatch.isSymbolsMatchingEnabled;
  }
  ...
  private void finishInitialFileOpenWarmupIfReady(final int token) {
    if (!editor.loadingCircle.isInitialFileOpenLoading
        || token != editor.loadingCircle.initialFileOpenToken) return;
    ...
    try (FileInputStream is = new FileInputStream(sourceFile);
        ByteArrayOutputStream out =
            new ByteArrayOutputStream(
                Math.min(
                    SAVE_LOG_MAX_CHARS, (int) Math.min(Integer.MAX_VALUE, sourceFile.length())))) {
      byte[] buffer = new byte[FILE_IO_BUFFER_SIZE];
      int remaining = SAVE_LOG_MAX_CHARS;
      int read;
      while (remaining > 0
          && (read = is.read(buffer, 0, Math.min(buffer.length, remaining))) != -1) {
        out.write(buffer, 0, read);
        remaining -= read;
      }
      String text =
          editor.binaryRender.isBinarySafeRenderingEnabled()
              ? editor.binaryRender.bytesToControlVisible(
                  out.toByteArray(), out.size(), fileCharset)
