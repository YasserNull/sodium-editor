...
public class ByteRangeLocator {
    ...
    private EditOp.RangeBytes computeByteRangeFromReadyIndex(File file, int sL, int sC, int eL, int eC) {
        if (editor == null || editor.fileIO == null || file == null || !editor.fileIO.isIndexReady) return null;
        ...
        try (RandomAccessFile raf = new RandomAccessFile(file, "r")) {
            return new EditOp.RangeBytes(
                    byteOffsetForChar(raf, startLine, sC),
                    byteOffsetForChar(raf, endLine, eC));
    ...
    public EditOp.RangeBytes computeByteRangeByScanning(File file, int sL, int sC, int eL, int eC) {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(file, "r")) {
            ...
            return new EditOp.RangeBytes(
                    byteOffsetForChar(raf, offsets[0], sC),
                    byteOffsetForChar(raf, offsets[1], eC));
        } catch (Exception e) {
            e.printStackTrace();
            return new EditOp.RangeBytes(0, 0);
        }
    ...
    private long byteOffsetForChar(RandomAccessFile raf, long lineStart, int charIndex) throws Exception {
