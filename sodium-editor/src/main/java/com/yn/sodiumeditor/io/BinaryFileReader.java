...
/**
 * BinaryFileReader handles reading binary files with safe conversion. This includes: - Reading
 * lines with binary-safe conversion - Reading byte slices - Reading character slices with streaming
...
public class BinaryFileReader {
  ...
  /**
   * Uses FileChannel + direct ByteBuffer read into a reused array. Eliminates ByteArrayOutputStream
   * and its internal byte[] copies.
   */
  public String readLineWithBinarySafe(
      ...
      long fileLen,
      Charset fileCharset,
      boolean binarySafeRenderingEnabled)
      throws Exception {
    ...
    synchronized (editor.fileIO.lineOffsetsLock) {
      ...
      start = editor.fileIO.lineOffsets[line];
          ...
          (line + 1 < editor.fileIO.lineOffsets.length)
              ? editor.fileIO.lineOffsets[line + 1]
              : fileLen;
    }
    if (start >= end) return "";
    ...
    if (binarySafeRenderingEnabled)
      return tokenConverter.bytesToControlVisible(sink, used, fileCharset);
  ...
  public String readLineSliceAtByte(
    ...
    int safeStart =
        (int) Math.max(0, Math.min(startChar, Math.min(Integer.MAX_VALUE, lineByteLen)));
    int safeEnd =
        (int) Math.max(safeStart, Math.min(endChar, Math.min(Integer.MAX_VALUE, lineByteLen)));
    ...
    if (binarySafeRenderingEnabled)
      return tokenConverter.bytesToControlVisible(buf, len, fileCharset);
    return new String(buf, 0, len, fileCharset);
  }

  // ── readLineSliceByChars ───────────────────────────────────────────────────
  /**
   * Key improvements: • Reuses ThreadLocal byte[], CharBuffer, StringBuilder • Early exit the
   * moment safeEnd chars collected (when !needTotalLength) • Single decoder instance per call (no
   * per-chunk recreation) • Binary-safe path uses lookup table, not escapeControlChar()
   */
  public StreamedCharSlice readLineSliceByChars(
      ...
      Charset fileCharset,
      boolean binarySafeRenderingEnabled,
      boolean binaryHexTokensEnabled)
      throws Exception {
        ...
        for (int i = 0; i < rem; i++) {
          ...
          if (!sliceDone && charIndex >= safeStart) {
            ...
            if (binarySafeRenderingEnabled && (c < 0x20 || c == 0x7F)) {
              ...
              sb.append(
                  b < BYTE_TOKEN.length ? BYTE_TOKEN[b] : tokenConverter.escapeControlChar(c));
            } else {
              sb.append(c);
            }
          }
    ...
    return new StreamedCharSlice(sb.toString(), needTotalLength ? charIndex : -1);
  }
