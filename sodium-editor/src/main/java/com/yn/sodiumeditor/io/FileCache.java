...
public class FileCache {
  ...
  public void populateDirectLinesForRange(
      int startLine, int endLineInclusive, Map<Integer, String> out) {
    ...
    try (RandomAccessFile raf = new RandomAccessFile(fileIO.sourceFile, "r")) {
      ...
      for (int cur = start; cur <= end; cur++) {
        ...
        if (editor.windowRender.shouldStreamLineLength(lineLen)) {
          editor.textRender.computeStreamedSliceBounds(
              null, cur, lineLen, editor.windowRender.streamedSliceTmp);
          ...
          if (editor.windowRender.isSingleByteCharset()) {
            ...
            StreamedCharSlice slice =
                editor.fileIO.readLineSliceByChars(raf, lineStart, sS, sE, true);
            ln = slice.text;
            editor.windowRender.setStreamedLineInfo(cur, slice.length, sS);
          }
        } else {
