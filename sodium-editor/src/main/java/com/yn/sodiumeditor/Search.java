...
public class Search {
  ...
  public boolean goToSearchMatch(boolean forward) {
    ...
    editor.ensureLineInWindow(match.line, true);
    editor.setCursorPosition(match.line, match.start);
    return true;
  }
  ...
  public SearchMatch findNextSearchMatchInRange(
    ...
    while (true) {
      ...
      if (line < editor.windowStartLine || line >= editor.windowStartLine + editor.linesWindow.size()) {
        if (editor.isIndexReady && editor.sourceFile != null && editor.sourceFile.exists()) {
          ...
          editor.populateDirectLinesForRange(rangeStart, rangeEnd, direct);
        }
      }
  ...
  public boolean selectSearchMatchAtCursorOrNext() {
    ...
    if (atCursor != null) {
      editor.ensureLineInWindow(atCursor.line, true);
