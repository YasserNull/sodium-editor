...
public void drawWhitespaceGuidesForRangeRtl(
    ...
    List<TextRender.HighlightSpan> syntaxSpans = getWhitespaceGuideSyntaxSpans(line, globalLine);
    boolean hasSyntaxSpans = !syntaxSpans.isEmpty();
    int syntaxIndex = 0;
    TextRender.HighlightSpan activeSyntax =
    ...
    for (int run = 0; run < runCount; run++) {
      ...
      for (int i = 0; i < runLen; i++) {
        ...
        float adv =
            (c == '\t')
                ? editor.getVisualTabWidth(editor.textRender.paint)
                : editor.getCharAdvanceWidth(c, whitespaceWidthBuffer[i], editor.textRender.paint);
        runWidth += adv;
      }
  ...
  public void drawWhitespaceGuidesForLine(Canvas canvas, String line, int globalLine, float y) {
    ...
    List<TextRender.HighlightSpan> visualSpans = editor.highlightCache.get(globalLine);
    if (visualSpans == null) {
    ...
    if (!visualSpans.isEmpty()) {
      for (TextRender.HighlightSpan span : visualSpans) {
        ...
        if (span.start > lastEnd) {
          currentX = drawWhitespaceGuidesSegment(
              ...
              editor.textRender.paint, syntaxSpans, hasSyntaxSpans, editor.textRender.whitespaceDrawState, rtlWidth);
        }

        currentX = drawWhitespaceGuidesSegment(
            canvas, line, span.start, safeSpanEnd, currentX, y,
            span.paint, syntaxSpans, hasSyntaxSpans, editor.textRender.whitespaceDrawState, rtlWidth);
        lastEnd = safeSpanEnd;
      }
    }
  ...
  public void drawWhitespaceGuidesForSegment(
    ...
    boolean hasSyntaxSpans = !syntaxSpans.isEmpty();
    editor.textRender.whitespaceDrawState.syntaxIndex = 0;
    boolean mirrorRtl = editor.textRender.isRtl && !editor.textRender.isMixedDirectionText(line, start, end);
  ...
  public List<TextRender.HighlightSpan> getWhitespaceGuideSyntaxSpans(String line, int globalLine) {
    List<TextRender.HighlightSpan> syntaxSpans = null;
    ...
    List<TextRender.HighlightSpan> spans = editor.highlightCache.get(globalLine);
    if (spans == null) {
