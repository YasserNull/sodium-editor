...
public class ImeScanner {
  ...
  public ImeContext buildImeContext(int beforeChars, int afterChars) {
    ...
    try {
      ...
      CursorTarget start =
          moveCursorByCharsForIme(editor.cursor.cursorLine, editor.cursor.cursorChar, -before, raf);
      ...
      CursorTarget end =
          moveCursorByCharsForIme(editor.cursor.cursorLine, editor.cursor.cursorChar, after, raf);
  ...
  public String getImeTextBeforeCursor(int length) {
    ...
    try {
      ...
      CursorTarget start =
          moveCursorByCharsForIme(editor.cursor.cursorLine, editor.cursor.cursorChar, -length, raf);
      long moveMs = android.os.SystemClock.uptimeMillis() - moveStartMs;
      long rangeStartMs = android.os.SystemClock.uptimeMillis();
      String result =
          buildRangeTextForIme(
              start, new CursorTarget(editor.cursor.cursorLine, editor.cursor.cursorChar), raf);
      long rangeMs = android.os.SystemClock.uptimeMillis() - rangeStartMs;
      return result;
    } finally {
      if (raf != null) {
  ...
  public String getImeTextAfterCursor(int length) {
    ...
    try {
      ...
      String result =
          buildRangeTextForIme(
              new CursorTarget(editor.cursor.cursorLine, editor.cursor.cursorChar), end, raf);
      long rangeMs = android.os.SystemClock.uptimeMillis() - rangeStartMs;
      return result;
    } finally {
      if (raf != null) {
  ...
  private RandomAccessFile openImeRandomAccessFile() {
    if (!editor.fileIO.isIndexReady
        ...
        || !editor.fileIO.sourceFile.exists()) return null;
    try {
  ...
  public CursorTarget moveCursorByCharsForIme(
  ...
  public String buildRangeTextForIme(
