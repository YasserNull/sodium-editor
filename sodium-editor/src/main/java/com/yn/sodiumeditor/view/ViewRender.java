...
final class ViewRender {
  ...
  // Render-related variables that were originally in SodiumEditorView
  ...
  void drawContent(Canvas canvas) {
    ...
    if (view.foldManager.isCodeFoldingEnabled) {
      ...
      for (int v = firstVisibleIndex; v <= lastVisibleIndex; v++) {
        ...
        view.paint.setUnderlineText(false); // Force disable underline before drawing
  ...
  void drawContentWrapped(Canvas canvas) {
    ...
    if (view.isFocused() && !view.isReadOnly && !view.selectionManager.hasSelection()) {
      ...
      if (cursorVisualIndex >= firstVisualIndex && cursorVisualIndex <= lastVisualIndex) {
        ...
        view.handlesManager.clearCursorHandleRect();
      }
    }

    if (view.selectionManager.hasSelection()) {
      int startVisual = view.getVisualIndexForLineAndChar(view.selectionManager.selStartLine, view.selectionManager.selStartChar);
      if (startVisual >= firstVisualIndex && startVisual <= lastVisualIndex) {
        ...
        float y = (startVisual - firstVisualIndex) * view.lineHeight + view.lineHeight;
        view.handlesManager.drawSelectionStartHandle(canvas, x, y, view.isRtl);
      } else {
        if (view.isRtl) view.handlesManager.clearRightHandleRect();
        else view.handlesManager.clearLeftHandleRect();
      }
      int endVisual = view.getVisualIndexForLineAndChar(view.selectionManager.selEndLine, view.selectionManager.selEndChar);
      if (endVisual >= firstVisualIndex && endVisual <= lastVisualIndex) {
        ...
        float y = (endVisual - firstVisualIndex) * view.lineHeight + view.lineHeight;
  ...
  void drawContentWrappedFallback(Canvas canvas, int wrapWidthPx) {
    int firstIndex = Math.max(0, (int) (view.scrollManager.scrollY / view.lineHeight));
    int lastIndex = firstIndex + (int) Math.ceil(view.getHeight() / view.lineHeight) + 5;
    final boolean drawDecorations = view.zoomManager.shouldDrawDecorations();
    ...
    if (view.foldManager.isCodeFoldingEnabled) {
      ...
      firstLine = view.mapVisibleIndexToGlobal(firstIndex);
      lastLine = view.mapVisibleIndexToGlobal(lastIndex);
    }

    view.maybeKickWindowLoad(firstLine);
    ...
    if (view.lineNumberManager.isHighlightCurrentLineInGutter()
      ...
      if (currentVisualIndex >= firstIndex && currentVisualIndex <= lastIndex) {
        float top = Math.round(currentVisualIndex * view.lineHeight - view.scrollManager.scrollY);
        float bottom = top + view.lineHeight;
        view.lineNumberManager.drawCurrentLineHighlightInGutter(canvas, top, bottom, view.highlightManager.currentLinePaint);
      }
    }
    ...
    canvas.translate(view.getTextStartX() - view.getEffectiveScrollX(), 0); // already translated by translateY
