...
public class PopupMenu {
  ...
  public void drawPopup(Canvas canvas) {
    ...
    if (actions.isEmpty()) {
      if ((popup.isFadingOut || popup.shouldKeepVisible())
          && !popup.popupLastDrawnActions.isEmpty()) {
    ...
    if (popup.isMinimalPopup || !editor.selection.hasSelection) {
      ...
      anchorX =
          editor.layout.getViewXForLineChar(
              cursorLineText, editor.cursor.cursorLine, editor.cursor.cursorChar);
      anchorY_top =
          editor.layout.getViewYTopForLineChar(editor.cursor.cursorLine, editor.cursor.cursorChar);
      ...
      anchorY_bottom =
          editor.layout.getViewYTopForLineChar(nEndLine, nEndChar) + editor.textRender.lineHeight;
    ...
    if (proposedLeft + totalWidth > editor.getWidth())
      proposedLeft = editor.getWidth() - totalWidth;
    if (proposedLeft < 0) proposedLeft = 0;
    ...
    if (popup.popupRippleActive
        && popup.popupRippleAlpha > 0f
        && !popup.popupRippleRect.isEmpty()) {
      int rippleAlpha =
          (int) (255f * Math.max(0f, Math.min(1f, popup.popupRippleAlpha * popup.popupAlpha)));
      ...
      canvas.drawCircle(
          popup.popupRippleX, popup.popupRippleY, popup.popupRippleRadius, popup.popupRipplePaint);
      canvas.restore();
