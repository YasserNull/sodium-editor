...
/**
 * LineNumberRender handles all line number rendering logic for SodiumEditor. This includes: -
 * Drawing line numbers (cached and direct) - Drawing current line number highlight - Writing
 * integers to char buffers
 */
public class LineNumberRender {
  ...
  public void drawLineNumbersDirectUnwrapped(
      Canvas canvas, int firstIdx, int lastIdx, int firstLine, int lastLine) {
    float lineNumX = getLineNumXUnwrapped();
    for (int i = firstLine; i <= lastLine; i++) {
      if (i == editor.cursor.cursorLine) continue;
      drawSingleLineNumber(
          canvas, i + 1, lineNumX, i * editor.textRender.lineHeight - editor.scroll.scrollY);
    }
  ...
  public void drawLineNumbersDirectWrapped(Canvas canvas, int firstV, int lastV) {
    ...
    for (int v = firstV; v <= lastV; v++) {
      ...
      drawSingleLineNumber(
          canvas, pos.line + 1, lineNumX, v * editor.textRender.lineHeight - editor.scroll.scrollY);
    }
  ...
  public void drawCurrentLineHighlightInGutter(Canvas canvas, float top, float bottom) {
    if (!lineNumber.showLineNumbers
        || !lineNumber.highlightCurrentLineInGutter
        || lineNumber.lineNumbersGutterWidth <= 0f
        || editor.selection.hasSelection) return;
  ...
  public void drawSingleLineNumber(Canvas canvas, int number, float x, float yOffset) {
    ...
    float y =
        Math.round(yOffset + editor.textRender.lineHeight - editor.textRender.paint.descent());
  ...
  public float getLineNumXWrapped() {
    return editor.textRender.isRtl
        ? lineNumber.getGutterStartX() + LineNumber.GUTTER_TEXT_PADDING
        : lineNumber.getGutterStartX()
            + lineNumber.lineNumbersGutterWidth
            - LineNumber.GUTTER_TEXT_PADDING;
