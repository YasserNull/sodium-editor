...
/**
 * BinaryLineDrawer handles fast binary line drawing. This includes: - Drawing binary lines with
 * token boxes - Drawing individual token boxes - Calculating X positions for binary tokens - Cursor
 * snapping for binary tokens
 */
public class BinaryLineDrawer {
  ...
  private static final ThreadLocal<float[]> TL_FLOAT_TMP_1 =
      ThreadLocal.withInitial(() -> new float[1]);
  ...
  /**
   * Update cached character width when text size or typeface changes. Call this whenever the main
   * paint changes.
   */
  ...
  // ── Cursor helpers ─────────────────────────────────────────────────────────
  public int snapBinaryCursor(
      String line, int index, int lineIndex, android.util.SparseArray<int[]> binaryTokenSpans) {
  ...
  /**
   * Optimized binary line drawing method with fast path. Uses cached character width and avoids all
  ...
  public void drawBinaryLine(
    ...
    drawBinaryLineSlice(
        canvas, line, globalLine, 0, line.length(), 0, y, defaultPaint, binaryTokenSpans);
  }
  ...
  public void drawBinaryLineSlice(
    ...
    if (spans == null || spans.length == 0) {
      drawBinaryTextRunWithFade(
          canvas, line, relStart, relEnd, 0f, y, defaultPaint, fadeStart, fadeEnd, fadeAlpha);
      return;
    }

    // Reuse temporary rectangle from editor
    RectF rect = editor.highlightRender.binaryTokenRect;
    ...
    for (int i = 0; i + 1 < spans.length; i += 2) {
      ...
      if (safeS > idx) {
        drawBinaryTextRunWithFade(
            canvas, line, idx, safeS, x, y, defaultPaint, fadeStart, fadeEnd, fadeAlpha);
        x += editor.textRender.measureTextWithVisualSpaces(line, idx, safeS, defaultPaint);
      }

      // Draw token box if enabled
      float tokenTextWidth =
          editor.textRender.measureTextWithVisualSpaces(line, safeS, safeE, defaultPaint);
      float tokenTotalWidth = tokenTextWidth + (padX * 2f);

      if (binaryTokenBoxEnabled) {
        rect.set(x, boxTop, x + tokenTotalWidth, boxBottom);
        canvas.drawRoundRect(
            rect, binaryTokenCornerRadius, binaryTokenCornerRadius, binaryTokenFillPaint);
        canvas.drawRoundRect(
            rect, binaryTokenCornerRadius, binaryTokenCornerRadius, binaryTokenStrokePaint);
      }
    ...
    if (idx < relEnd) {
      drawBinaryTextRunWithFade(
          canvas, line, idx, relEnd, x, y, defaultPaint, fadeStart, fadeEnd, fadeAlpha);
    }
  }
  ...
  private float drawBinaryTextRunWithFade(
    ...
    if (!hasFade || end <= fadeStart || start >= fadeEnd) {
      if (line.indexOf('\t', start) < end) {
        return editor.textLineDraw.drawTextSegmentWithVisualSpaces(
            canvas, line, start, end, x, y, paint, 1f);
      }
    ...
    if (start < beforeEnd) {
      if (line.indexOf('\t', start) < beforeEnd) {
        currentX +=
            editor.textLineDraw.drawTextSegmentWithVisualSpaces(
                canvas, line, start, beforeEnd, currentX, y, paint, 1f);
      } else {
    ...
    if (afterStart < end) {
      if (line.indexOf('\t', afterStart) < end) {
        currentX +=
            editor.textLineDraw.drawTextSegmentWithVisualSpaces(
                canvas, line, afterStart, end, currentX, y, paint, 1f);
      } else {
