...
/**
 * BinaryRender is the main entry point for binary-safe rendering. This class delegates to
 * specialized components: - BinaryTokenConverter: Token conversion and lookup tables -
 * BinaryFileReader: File reading operations - BinaryLineDrawer: Drawing operations
 */
public class BinaryRender {
  ...
  private boolean savedSymbolsMatchingEnabled;
  ...
  public void applyBinaryFileFeaturePolicy(boolean active) {
    ...
    if (active) {
      ...
      editor.highlight.isSyntaxHighlightingEnabled = false;
  ...
  private void saveFeatureStateForBinaryFile() {
    savedSyntaxHighlightingEnabled = editor.highlight.isSyntaxHighlightingEnabled;
    ...
    savedSymbolsMatchingEnabled = editor.symbolsMatch.isSymbolsMatchingEnabled;
  ...
  private void restoreFeatureStateAfterBinaryFile() {
    editor.highlight.isSyntaxHighlightingEnabled = savedSyntaxHighlightingEnabled;
    editor.highlight.clearHighlightCaches();
    ...
    editor.symbolsMatch.setSymbolsMatchingEnabled(savedSymbolsMatchingEnabled);
  ...
  public String bytesToControlVisibleAndCacheSpans(
    ...
    return tokenConverter.bytesToControlVisibleAndCacheSpans(
        buf, len, lineIndex, binaryTokenSpans, charset);
  }

  public String rawBytesToControlVisibleAndCacheSpans(byte[] buf, int len, int lineIndex) {
    return tokenConverter.rawBytesToControlVisibleAndCacheSpans(
        buf, len, lineIndex, binaryTokenSpans);
  }

  // ── File Reading ───────────────────────────────────────────────────────────
  public String readLineWithBinarySafe(
      java.io.RandomAccessFile raf, int line, long fileLen, java.nio.charset.Charset fileCharset)
      throws Exception {
    return fileReader.readLineWithBinarySafe(
        raf, line, fileLen, fileCharset, binarySafeRenderingEnabled);
  }
  ...
  public String readLineSliceAtByte(
      ...
      throws Exception {
    return fileReader.readLineSliceAtByte(
        raf, lineStart, lineByteLen, startChar, endChar, fileCharset, binarySafeRenderingEnabled);
  }
  ...
  public StreamedCharSlice readLineSliceByChars(
      ...
      int startChar,
      int endChar,
      boolean needTotalLength,
      java.nio.charset.Charset fileCharset)
      throws Exception {
    return fileReader.readLineSliceByChars(
        raf,
        lineStart,
        startChar,
        endChar,
        ...
        tokenConverter.isBinaryHexTokensEnabled());
  }
  ...
  public float getXForCharBinary(
      String line, int charIndex, android.graphics.Paint paint, int[] spans, float padX) {
  ...
  public void drawBinaryLineSlice(
    ...
    lineDrawer.drawBinaryLineSlice(
        canvas, line, globalLine, relStart, relEnd, sliceStart, y, defaultPaint, binaryTokenSpans);
  }
  ...
  public android.graphics.Paint getBinaryTokenFillPaint() {
    return lineDrawer.getBinaryTokenFillPaint();
  }

  public android.graphics.Paint getBinaryTokenStrokePaint() {
    return lineDrawer.getBinaryTokenStrokePaint();
  }
}
