...
/**
 * TextLineDraw handles all text line drawing operations for SodiumEditor. This includes: - Drawing
 * text segments with fade effects - Drawing text with visual spaces - Drawing underlines with fade
 * effects - Drawing delete animations
 */
public class TextLineDraw {
  ...
  public float drawTextSegmentWithFadeAndUnderlines(
      ...
      int fadeEnd,
      float fadeAlpha,
      @Nullable List<TextRender.UnderlineSpan> underlines,
      float lineTop,
      float lineBottom) {
    if (start >= end) return 0f;
  ...
  public void drawUnderlineSegmentWithFade(
    ...
    Paint tmpPaintToUse =
        isPath
            ? editor.pathUnderline.getPathUnderlinePaint()
            : editor.urlUnderline.urlUnderlineTmpPaint;
    ...
    if (fadeSegStart < fadeSegEnd) {
      ...
      float w =
          editor.textRender.measureTextWithVisualSpaces(line, fadeSegStart, fadeSegEnd, textPaint);
      if (w > 0f) canvas.drawLine(currentX, underlineY, currentX + w, underlineY, tmpPaintToUse);
      currentX += w;
    }
  ...
  public float drawTextSegmentWithVisualSpaces(
    ...
    for (int i = 0; i < len; i++) {
      ...
      float adv =
          editor.textRender.getCharAdvanceWidth(c, editor.view.measureWidthBuffer[i], segmentPaint);
  ...
  public void drawDeleteAnimationForSegment(
    ...
    float x =
        editor.textRender.measureTextWithVisualSpaces(line, segStart, at, editor.textRender.paint);
    Paint ghostPaint =
