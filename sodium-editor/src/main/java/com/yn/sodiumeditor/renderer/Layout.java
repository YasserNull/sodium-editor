...
/**
 * Layout handles all layout-related logic for SodiumEditor. This includes: - RTL/LTR text direction
 * management - Padding and margin calculations - Text area width/height measurements - Line base X
 * position calculations for RTL - Segment base X position calculations for RTL - Layout direction
 * changes
 */
public class Layout {
  ...
  public void setLayoutDirection(boolean Direction) {
    ...
    isRtl = Direction;
    editor.lineNumber.lineNumbersPaint.setTextAlign(
        editor.textRender.isRtl ? Paint.Align.LEFT : Paint.Align.RIGHT);
  ...
  public boolean isWhitespaceAtX(String line, int globalLine, float x) {
    ...
    List<HighlightRender.HighlightSpan> spans = editor.highlight.highlightCache.get(globalLine);
    if (spans == null) {
    ...
    if (pos < len) {
      if (hitTestWhitespaceSegment(
          line, pos, len, globalLine, x, editor.textRender.paint, eps, currentX, prevWhitespace)) {
        if (isGuideHitOnWhitespaceBoundary(line, x)) return false;
        return true;
      }
    }
  ...
  public boolean isGuideHitOnWhitespaceBoundary(String line, float x) {
    if (!editor.layout.lastHitAdvance.hit || !editor.layout.lastHitAdvance.isWhitespace)
      return false;
  ...
  /**
   * Get the RTL line base X position. This is used to align text to the right in RTL mode.
   *
  ...
  public float getRtlLineBaseX(@Nullable String line, int globalLine) {
    ...
    float w = editor.highlight.measureHighlightedSegmentWidth(line, globalLine, 0, logicalLen);
  ...
  public float convertXToRtl(float x, String line, int globalLine) {
    ...
    float lineWidth =
        editor.highlight.measureHighlightedSegmentWidth(
            line, globalLine, 0, editor.view.getLogicalLineLength(globalLine, line));
  ...
  /**
   * Get the text start X position (where text drawing begins). This accounts for gutter width and
   * padding based on the current layout direction.
   */
  ...
  public float getViewXForLineChar(String line, int globalLine, int ch) {
    ...
    float x =
        editor.textRender.measureTextWithVisualSpaces(
            line, segStart, safeChar, editor.textRender.paint);
