...
public class CodeFold {
    ...
    public int getHiddenLineCount() {
        ...
        if (total <= 0) total = editor.windowStartLine + editor.linesWindow.size();
        int hidden = 0;
        for (int[] interval : foldIntervals) {
            int s = interval[0];
    ...
    public int mapVisibleIndexToGlobal(int visibleIndex) {
        ...
        int totalLines = editor.getLinesCount();
                    if (totalLines <= 0) totalLines = editor.windowStartLine + editor.linesWindow.size();        
    ...
    private String getLineTextForFoldScan(int line, @Nullable RandomAccessFile raf) {
        if (line < 0) return null;
        String mod = editor.modifiedLines.get(line);
        if (mod != null) return mod;
        if (line >= editor.windowStartLine && line < editor.windowStartLine + editor.linesWindow.size()) {
            String text = editor.getLineFromWindowLocal(line - editor.windowStartLine);
            return (text != null) ? text : "";
        }
        if (raf != null && editor.isIndexReady) {
            long offset;
            synchronized (editor.lineOffsetsLock) {
                if (line < 0 || line >= editor.lineOffsets.length) return null;
                offset = editor.lineOffsets[line];
            }
            try {
    ...
    private FoldRange findIndentFoldRangeForLine(int line, @Nullable RandomAccessFile raf) {
        ...
        int totalLines = editor.getLinesCount();
        if (totalLines <= 0) totalLines = Math.max(line + 1, editor.windowStartLine + editor.linesWindow.size());
    ...
    private FoldToken findFoldTokenInLine(String line, int startIndex) {
        ...
        while (i < len) {
            ...
            if (editor.isLineCommentStart(line, i)) {
                inLineComment = true;
                break;
            }
    ...
    private int findBlockCommentEndLine(int startLine, int startChar, @Nullable RandomAccessFile raf) {
        ...
        int totalLines = editor.getLinesCount();
        if (totalLines <= 0) totalLines = Math.max(startLine + 1, editor.windowStartLine + editor.linesWindow.size());
    ...
    private FoldMatch findMatchingBracketFrom(int startLine, int startChar, char openBracket, @Nullable RandomAccessFile raf) {
        ...
        while (true) {
            while (i < line.length()) {
                ...
                if (c == openBracket && !editor.isEscaped(line, i)) {
                    depth++;
                } else if (c == closeBracket && !editor.isEscaped(line, i)) {
                    depth--;
                    if (depth == 0) {
