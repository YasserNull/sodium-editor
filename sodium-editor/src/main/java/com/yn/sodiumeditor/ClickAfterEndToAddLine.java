...
public class ClickAfterEndToAddLine {
  ...
  public boolean isClickAfterEnd(int visibleIndex, int totalVisible) {
    if (editor.isEof && visibleIndex >= editor.windowStartLine + editor.linesWindow.size() && !editor.linesWindow.isEmpty()) {
      return true;
    }
  ...
  public boolean handleClickAfterEnd(int visibleIndex, int totalVisible) {
    ...
    int lastLineIndex = editor.windowStartLine + editor.linesWindow.size() - 1;
  ...
  public void handleDefaultAfterEnd() {
    editor.cursor.cursorLine = editor.windowStartLine + editor.linesWindow.size() - 1;
    String lastLineText = editor.getLineTextForRender(editor.cursor.cursorLine);
    editor.cursor.cursorChar = lastLineText.length();
  }
}
