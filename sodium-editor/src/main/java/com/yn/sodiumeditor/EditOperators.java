...
public class EditOperators {
    ...
    try {
      ...
      if (root.has("cursorLine") && root.has("cursorChar")) {
        ...
        if (root.optBoolean("hasSelection", false)) {
          ...
          editor.setCursorPosition(cLine, cChar);
        }
      }
  ...
  public void applyEditForUndoRedo(
    ...
    editor.setCursorPosition(cursorLine, cursorChar);
        ...
        () -> {
          ...
          for (EditOp op : ops) {
            ...
            if (!rewriteReplaceRangeBlocking(
                editor.sourceFile, op.startLine, op.startChar, op.endLine, op.endChar, op.insertedText)) {
              ok = false;
              break;
            }
          }
          ...
          editor.post(
              () -> {
                if (!success) {
                  ...
                  synchronized (editor.modifiedLines) {
                    editor.modifiedLines.clear();
                  }
                  lineCountDelta = 0;
                  editor.lineNumber.invalidateLineNumberCache();
  ...
  public boolean rewriteReplaceRangeBlocking(
    ...
    try {
      ...
      editor.sourceFile = inFile;
      synchronized (editor.lineOffsetsLock) {
        editor.lineOffsets = new long[0];
      }
      editor.isIndexReady = false;
      editor.isIndexBuilding = false;
      editor.isIndexDisabled = false;
      editor.indexDisabledPath = null;
      editor.indexDisabledFileLength = -1L;
      editor.ioHandler.post(editor::buildFileIndex);
      return true;
    } catch (Exception e) {
      return false;
    }
            ...
            editor.post(
                () -> {
                  ...
                  if (inFile != null) {
                    ...
                    editor.sourceFile = outFile;
                  }
                  editor.isFileCleared = false;

                  synchronized (editor.modifiedLines) {
                    editor.modifiedLines.clear();
                  }
                  synchronized (editor.lineWidthCache) {
                    editor.lineWidthCache.clear();
                  }
                  editor.currentMaxWindowLineWidth = 0f;
                  editor.globalMaxLineWidth = 0f;
                  ...
                  boolean cursorInsideWindow =
                      (editor.cursor.cursorLine >= editor.windowStartLine
                          && editor.cursor.cursorLine < editor.windowStartLine + editor.linesWindow.size());

                  if (cursorInsideWindow) {
                    // النافذة الحالية تم تعديلها مسبقاً (fast path)، فقط أعد حساب العرض وحدث الرسم.
                    synchronized (editor.linesWindow) {
                      editor.isEof = editor.linesWindow.size() < editor.windowSize + (editor.prefetchLines * 2);
                    }
                    ...
                    int targetStart = Math.max(0, editor.cursor.cursorLine - editor.prefetchLines);
                    editor.loadWindowAround(
                        targetStart,
                        () -> {
  ...
  public RangeBytes computeByteRangeFastOrScan(File file, int sL, int sC, int eL, int eC) {
    ...
    if (editor.isIndexReady && file != null) {
  ...
  public RangeBytes computeByteRangeUsingIndex(File file, int sL, int sC, int eL, int eC) {
    try (RandomAccessFile raf = new RandomAccessFile(file, "r")) {
      ...
      synchronized (editor.lineOffsetsLock) {
        if (!editor.isIndexReady) return null;
        if (sL < 0 || eL < 0) return null;
        if (sL >= editor.lineOffsets.length || eL >= editor.lineOffsets.length) return null;
        startLineByte = editor.lineOffsets[sL];
        endLineByte = editor.lineOffsets[eL];
      }

      String startLineText = editor.readLineUtf8AtByte(raf, startLineByte);
      String endLineText = (eL == sL) ? startLineText : editor.readLineUtf8AtByte(raf, endLineByte);
  ...
  public void insertCharAtCursor(char c) {
    ...
    editor.invalidatePendingIOForEdit();
    editVersion.incrementAndGet();
    ...
    editor.ensureLineInWindow(editor.cursor.cursorLine, true);
    if (editor.isWindowLoading
        && (editor.cursor.cursorLine < editor.windowStartLine || editor.cursor.cursorLine >= editor.windowStartLine + editor.linesWindow.size())) {
      editor.post(() -> insertCharAtCursor(c));
      return;
    }

    int localIdx = editor.cursor.cursorLine - editor.windowStartLine;
    if (localIdx < 0 || localIdx >= editor.linesWindow.size()) {
      synchronized (editor.linesWindow) {
        if (editor.linesWindow.isEmpty()) editor.linesWindow.add("");
      }
      localIdx = Math.max(0, Math.min(localIdx, editor.linesWindow.size() - 1));
    }
    ...
    synchronized (editor.linesWindow) {
      ...
      if (c == '\n') {
        ...
        Float oldWidth = editor.lineWidthCache.get(editor.cursor.cursorLine);

        editor.updateLocalLine(localIdx, before);
        editor.linesWindow.add(localIdx + 1, after);

        editor.modifiedLines.put(editor.cursor.cursorLine, before);
        editor.modifiedLines.put(editor.cursor.cursorLine + 1, after);
        ...
        if (oldWidth != null && oldWidth >= editor.currentMaxWindowLineWidth)
          editor.recalculateMaxLineWidthAsync();
        ...
        editor.modifiedLines.put(editor.cursor.cursorLine, modified);
        if (editor.codeFold.isCodeFoldingEnabled) {
          editor.codeFold.invalidateFoldRangeForLine(editor.cursor.cursorLine);
        }
        ...
        synchronized (editor.lineWidthCache) {
          editor.lineWidthCache.put(editor.cursor.cursorLine, newWidth);
        }
        editor.currentMaxWindowLineWidth = Math.max(editor.currentMaxWindowLineWidth, newWidth);
        editor.globalMaxLineWidth = Math.max(editor.globalMaxLineWidth, editor.currentMaxWindowLineWidth);
      }
      editor.invalidate();
      editor.keepCursorVisibleHorizontally();
    }
  ...
  public void deleteCharAtCursor() {
    ...
    int localIdx = editor.cursor.cursorLine - editor.windowStartLine;
    if (localIdx < 0 || localIdx >= editor.linesWindow.size()) return;
    ...
    synchronized (editor.linesWindow) {
      ...
      if (editor.cursor.cursorChar > 0) {
        ...
        editor.ensureLineInWindow(prevGlobal, true);
        int prevLocal = prevGlobal - editor.windowStartLine;
        if (prevLocal < 0 || prevLocal >= editor.linesWindow.size()) return;
        ...
        editor.modifiedLines.put(prevGlobal, merged);
        if (editor.codeFold.isCodeFoldingEnabled) {
        ...
        if (localIdx < editor.linesWindow.size()) editor.linesWindow.remove(localIdx);
  ...
  public void deleteForwardAtCursor() {
    ...
    synchronized (editor.linesWindow) {
      ...
      if (editor.cursor.cursorChar < base.length()) {
        ...
        if (editor.isEof && nextGlobal >= editor.windowStartLine + editor.linesWindow.size()) return;

        editor.ensureLineInWindow(nextGlobal, true);
        int nextLocal = nextGlobal - editor.windowStartLine;
        if (nextLocal >= 0 && nextLocal < editor.linesWindow.size()) {
          String next = editor.getLineFromWindowLocal(nextLocal);
          if (next == null) next = "";
          String merged = base + next;
          editor.updateLocalLine(localIdx, merged);
          editor.linesWindow.remove(nextLocal);
          editor.modifiedLines.put(editor.cursor.cursorLine, merged);
          if (editor.codeFold.isCodeFoldingEnabled) {
  ...
  public void insertTextAtCursor(String text) {
    ...
    // path.
    if (editor.sourceFile != null && !editor.isFileCleared && isLargePasteText(text)) {
      ...
      final File inFile = editor.sourceFile;
    ...
    int local = editor.cursor.cursorLine - editor.windowStartLine;
    if (local < 0 || local >= editor.linesWindow.size()) {
      synchronized (editor.linesWindow) {
        if (editor.linesWindow.isEmpty()) {
          editor.linesWindow.add("");
          local = 0;
        } else local = Math.max(0, Math.min(local, editor.linesWindow.size() - 1));
      }
    }
    ...
    synchronized (editor.linesWindow) {
      ...
      if (parts.length == 1) {
        ...
        editor.lineWidthCache.remove(editor.cursor.cursorLine);
        editor.cursor.cursorChar += parts[0].length();
      } else {
        editor.lineWidthCache.clear();
        String firstLine = left + parts[0];
        editor.updateLocalLine(local, firstLine);
        editor.modifiedLines.put(editor.cursor.cursorLine, firstLine);
        if (editor.codeFold.isCodeFoldingEnabled && editor.containsBracketChars(parts[0])) {
          editor.bracketCache.invalidateLines(editor.cursor.cursorLine, editor.cursor.cursorLine);
          editor.codeFold.invalidateFoldRangeForLine(editor.cursor.cursorLine);
        }
        ...
        if (!linesToInsert.isEmpty()) editor.linesWindow.addAll(local + 1, linesToInsert);
        for (int i = 0; i < linesToInsert.size(); i++) {
          editor.modifiedLines.put(editor.cursor.cursorLine + 1 + i, linesToInsert.get(i));
