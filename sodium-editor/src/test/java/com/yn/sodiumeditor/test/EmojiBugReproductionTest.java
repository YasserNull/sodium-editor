...
/**
 * Regression test for the bug where committing an emoji immediately after a word would cause the
 * word to be replaced (disappear).
 */
...
public class EmojiBugReproductionTest {
  ...
  public void testTextDisappearingAfterEmojis() {
    ...
    assertEquals(
        "Initial text should be 'Hello'", initialText, editor.windowRender.getLineTextForRender(0));
    ...
    // The fix ensures emojis are not considered "words" for replacement in
