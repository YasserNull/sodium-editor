...
public class RealFileDeleteLineTest {
  ...
  public void deleteLineFromRealFile_doesNotReappearAfterWindowReloadCheck() throws Exception {
    File input =
        new File(RuntimeEnvironment.getApplication().getCacheDir(), "real_delete_test.txt");
    writeUtf8(input, "AAA\nBBB\nCCC");
    ...
    // Simulate: delete characters of line 2 ("BBB"), then delete the now-empty line by backspacing
    // at column 0.
    ...
    editor.editOperators
        .deleteCharAtCursor(); // backspace at start => merge with previous => delete line
    drainUiAndIo();
