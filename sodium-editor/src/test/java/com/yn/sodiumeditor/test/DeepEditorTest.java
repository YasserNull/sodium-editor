...
public class DeepEditorTest {
    ...
    public void testSimulatedUserDeletionAndMerge() {
        ...
        try {
            ...
            TestFileLogger.append(logFile, "Line 0 content: " + editor.windowRender.getLineTextForRender(0));
            TestFileLogger.append(logFile, "Line 1 content: " + editor.windowRender.getLineTextForRender(1));
        } catch (Exception ignored) {
        }

        assertEquals(1, editor.view.getLinesCount());
        assertEquals("ABCDEF", editor.windowRender.getLineTextForRender(0));
        assertEquals("Deleted line should render empty (no phantom text)", "", editor.windowRender.getLineTextForRender(1));
