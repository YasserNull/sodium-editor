...
public final class TerminalEmulator {
  ...
  /**
   * Holds the bit flags which arguments are sub parameters (after a colon) - bit N is set if <code>
   * mArgs[N]</code> is a sub parameter.
   */
  ...
  /**
   * Current foreground, background and underline colors. Can either be a color index in [0,259] or
   * a truecolor (24-bit) value. For a 24-bit value the top byte (0xff000000) is set.
   *
  ...
  public void processCodePoint(int b) {
    // The Application Program-Control (APC) string might be arbitrary non-printable characters, so
    // handle that early.
            ...
            if (b == 'p') {
              ...
              if (mode == 47 || mode == 1047 || mode == 1049) {
                ...
                if (internalBit != -1) {
                  ...
                  Logger.logError(
                      mClient, LOG_TAG, "Got DECRQM for unrecognized private DEC mode=" + mode);
              ...
              if (part.length() % 2 == 0) {
                ...
                for (int i = 0; i < part.length(); i += 2) {
                  try {
                    c =
                        (char)
                            Long.decode("0x" + part.charAt(i) + "" + part.charAt(i + 1))
                                .longValue();
                ...
                if (responseValue == null) {
                  switch (trans) {
                    ...
                    default:
                      Logger.logWarn(
                          mClient, LOG_TAG, "Unhandled termcap/terminfo name: '" + trans + "'");
                  }
  ...
  /**
   * Process byte while in the {@link #ESC_CSI_UNSUPPORTED_PARAMETER_BYTE} or {@link
   * #ESC_CSI_UNSUPPORTED_INTERMEDIATE_BYTE} escape state.
   *
   * <p>Parse unsupported parameter, intermediate and final bytes but ignore them.
   *
   * <p>> For Control Sequence Introducer, ... the ESC [ is followed by > - any number (including
   * none) of "parameter bytes" in the range 0x30–0x3F (ASCII 0–9:;<=>?), > - then by any number of
   * "intermediate bytes" in the range 0x20–0x2F (ASCII space and !"#$%&'()*+,-./), > - then finally
   * by a single "final byte" in the range 0x40–0x7E (ASCII @A–Z[\]^_`a–z{|}~).
   *
   * <p>- https://en.wikipedia.org/wiki/ANSI_escape_code#Control_Sequence_Introducer_commands -
  ...
  private void doCsiUnsupportedParameterOrIntermediateByte(int b) {
    if (mEscapeState == ESC_CSI_UNSUPPORTED_PARAMETER_BYTE && b >= 0x30 && b <= 0x3F) {
      ...
      // Calling `unknownSequence()` would log an error with only a final byte, so ignore it for
      // now.
      finishSequence();
    } else {
      unknownSequence(b);
    ...
    switch (b) {
      ...
      case 's':
        ...
        for (int i = 0; i <= mArgIndex; i++) {
          ...
          if (internalBit == -1) {
            Logger.logWarn(
                mClient, LOG_TAG, "Ignoring request to save/recall decset bit=" + externalBit);
          } else {
            if (b == 's') {
    ...
    switch (value) {
      ...
      case 52: // Manipulate Selection Data. Skip the optional first selection parameter(s).
        ...
        try {
          ...
          Logger.logError(
              mClient, LOG_TAG, "OSC Manipulate selection, invalid string '" + textParameter + "");
        }
        break;
      case 104:
  ...
  private void scrollDownOneLine() {
    ...
    if (mLeftMargin != 0 || mRightMargin != mColumns) {
      ...
      mScreen.blockSet(
          mLeftMargin, mBottomMargin - 1, mRightMargin - mLeftMargin, 1, ' ', currentStyle);
    } else {
   ...
   *
   * <p>Parameter characters modify the action or interpretation of the sequence. Originally you can
   * use up to 16 parameters per sequence, but following at least xterm and alacritty we use a
   * common space for parameters and sub-parameters, allowing 32 in total.
   *
   * <p>All parameters are unsigned, positive decimal integers, with the most significant digit sent
   * first. Any parameter greater than 9999 (decimal) is set to 9999 (decimal). If you do not
   * specify a value, a 0 value is assumed. A 0 value or omitted parameter indicates a default value
   * for the sequence. For most sequences, the default value is 1.
   *
   * <p>References: <a href="https://vt100.net/docs/vt510-rm/chapter4.html#S4.3.3">VT510 Video
   * Terminal Programmer Information: Control Sequences</a> <a
   * href="https://github.com/alacritty/vte/issues/22">alacritty/vte: Implement colon separated CSI
   * parameters</a>
   */
